            ...
            <main class="flex-1 p-4 overflow-y-auto">
                @foreach($users as $user)
                    <div class="bg-white rounded-lg shadow-lg p-4 mb-4">
                        ...
                        @if(!$user->contract)
                            <a href="{{ route('admin.contracts.export', $user) }}" class="bg-blue-500 text-white px-3 py-2 rounded-lg">Export contract</a>
                            <a @click="uploadOpen = true" class="bg-blue-500 text-white px-3 py-2 rounded-lg">{{ __('global.upload_contract') }}</a>
                        @elseif($user->contract->approved_at === null)
                            <a @click="contractOpen = true" class="bg-green-500 text-white px-3 py-2 rounded-lg">{{ __('global.approve_contract') }}</a>
                        @else

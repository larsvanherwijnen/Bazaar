<div class="flex flex-col space-y-5">
    @if(auth()->check() && auth()->user()->id == $advert->user_id)
        <div class="flex space-x-4 justify-end">
            <a href="{{ route('my-account.adverts.edit', $advert) }}" class="bg-yellow-400 rounded p-1 text-sm"><i class="fa-solid fa-pen-to-square mr-2"></i>{{__('global.edit')}}</a>
            <form action="{{route('my-account.adverts.destroy', $advert)}}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-500 rounded p-1 text-sm"><i class="fa-solid fa-trash mr-2"></i>{{__('global.delete')}}</button>
            </form>
        </div>
    @endif
    <div class=" pb-5 flex justify-between">
        ...
        <div x-data="{ mainImage: '' }"
            ...
            @if($advert->type == \App\Enum\AdvertType::BIDDING && $advert->price != null)
                <span class="text-xl font-bold text-gray-900">€{{ $advert->price }}</span>
            @elseif($advert->type == \App\Enum\AdvertType::BIDDING)

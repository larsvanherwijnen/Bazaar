...
@section('content')
    ...
    <div class="flex justify-center mt-20">
        <div class="bg-white p-10 rounded shadow-md w-1/3">
            ...
            <form method="post" action="{{ route('my-account.adverts.store') }}" enctype="multipart/form-data"
                  class="space-y-4" x-data="{ advertType: '{{ old('type', 'Sale') }}' }">
                @csrf
                    ...
                    @foreach(\App\Enum\AdvertType::cases() as $type)
                        <div class="w-1/4 bg-gray-200 rounded flex flex-col items-center py-3 "
                            ...
                            <input type="checkbox" id="{{ $type }}" name="type"
                                   ...
                                   :checked="{{ $type == \App\Enum\AdvertType::SALE }}">
                ...
                <div>
                    ...
                    <label for="description"
                           class="block text-sm font-medium text-gray-700">{{ __('global.description') }}:</label>
                    <textarea id="description" name="description"
                              class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description') }}</textarea>
                </div>
                <div>
                    ...
                    @enderror
                    <label for="description"
                           class="block text-sm font-medium text-gray-700">{{ __('global.related_adverts') }}:</label>
                    <select class="js-example-basic-multiple mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                            name="relatedAdverts[]" multiple="multiple">
                ...
                <div id="image-preview" class="grid grid-cols-4 grid-rows-2 gap-2">
                    <div class="image-placeholder col-span-2 row-span-2 border border-gray-300 rounded overflow-hidden h-full">
                        <img src="https://placehold.co/600x400?text=Your+image+goes+here"
                ...
                <div x-show="advertType === 'Sale' || advertType === 'Rental' || advertType === 'Bidding'">
                    <div>
                        ...
                        <div class="text-red-500 mt-2 text-sm">{{ $message }}</div>
                        @enderror
                        <label for="price" class="block text-sm font-medium text-gray-700">{{__('advert.price')}}
                            :</label>
                ...
                <div x-show="advertType === 'Auction' || advertType === 'Bidding'">
                    ...
                    <label for="starting_price"
                           class="block text-sm font-medium text-gray-700">{{__('advert.starting_price')}}:</label>
                    <input type="number" id="starting_price" name="starting_price" value="{{ old('starting_price') }}"
                           class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
                <div x-show="advertType === 'Auction'">
                    <div>
                        ...
                        <label for="start_date"
                               class="block text-sm font-medium text-gray-700">{{__('advert.start_date')}}:</label>
                    ...
                    <div>
                        ...
                        <div class="text-red-500 mt-2 text-sm">{{ $message }}</div>
                        @enderror
                        <label for="end_date" class="block text-sm font-medium text-gray-700">{{__('advert.end_date')}}
                            :</label>
                ...
                <div>
                    <button type="submit"
                            class="w-full py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">{{__('global.submit')}}</button>
                </div>
            </form>
        </div>
    </div>
